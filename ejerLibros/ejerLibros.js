...
const programmingBooks = [
   ...
   let bookName = prompt("Ingrese un nombre:");

let book = bookName.toLowerCase();
let library = programmingBooks.map(student => student.toLowerCase());

let titleExists = library.includes(book);

if (titleExists) {
  alert("El título se encuentra en la lista de libros.");
} else {
  alert("El título no se encuentra en la lista de libros.");
}

//5

let vectorIndex = programmingBooks.indexOf("You Don't Know JS");
alert(`el libro se encuenta en la estanteria #${vectorIndex}`);